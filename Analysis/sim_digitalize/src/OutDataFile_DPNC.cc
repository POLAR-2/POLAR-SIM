...
OutDataFile_DPNC::OutDataFile_DPNC() {
    t_file_out_ = NULL;
...
bool OutDataFile_DPNC::open(const char* filename) {
    if (t_file_out_ != NULL)
        return false;
    ...
    t_modules_tree_->Branch("badch",            t_modules_.badch,           "badch[64]/I"            );
    ...
    t_modules_tree_->Branch("tunix",            &t_modules_.tunix,            "tunix/D"           );
...
void OutDataFile_DPNC::write_event(const Digi::GeneralEvent_T& general_event) {
    trigg_count_++;
        ...
        if (general_event.trig_accepted[i]) {
            ...
            for (int j = 0; j < 64; j++) {
                ...
                if (general_event.energy_adc[i][j] < 4094.0) {
                    t_modules_.badch[j] = 0;
                } else {
                    t_modules_.badch[j] = 256;
                }
            }
...
void OutDataFile_DPNC::write_tree() {
    if (t_file_out_ == NULL) {
        return;
    } else {
        t_file_out_->cd();
...
long int OutDataFile_DPNC::get_total_events() {
