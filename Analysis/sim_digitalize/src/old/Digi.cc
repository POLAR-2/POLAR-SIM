...
void Digi::energy_response(const vector<EventReader::HitsCol_T>& hitscol_vec, const Calib& calib_obj) {
    // energy -> ADC
...
// ==============================================================================
    ...
    for (size_t i = 0; i < hitscol_vec.size(); i++) {
        ...
        double rand_photon = digi_random_.Gaus(mean_photon, intrinsic_res * TMath::Sqrt(mean_photon));
        if (rand_photon > 0) {
...
//                     bar_optical_photon_ax[i][jx] += digi_random_.PoissonD(tmp_value);
                    double neigh_opt = digi_random_.Gaus(tmp_value, xtalk_res * TMath::Sqrt(tmp_value));
                    neigh_opt = round(neigh_opt);
                    if (neigh_opt < 0) neigh_opt = 0;
                    bar_optical_photon_ax[i][jx] += neigh_opt;
                }
