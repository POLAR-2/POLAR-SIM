...
bool OptionsManager::parse(int argc_par, char** argv_par) {
    ...
    while (idx < argc_par - 1) {
        ...
        if (cur_par_str[0] == '-') {
            ...
            if (simdata_filename.IsNull()) {
                simdata_filename = cur_par_str;
            } else {
                return false;
            }
        }
    ...
    if (output_type != "IHEP" && output_type != "DPNC") {
        return false;
    }
    return true;
}
...
void OptionsManager::print_help() {
    cout << "Usage:" << endl;
    cout << "  " << SW_NAME << " <simdata.root> -c <config.cfg> -t <IHEP|DPNC> -o <digiout.root> [-s <seed>]" << endl;
    cout << endl;
    cout << "Options:" << endl;
    cout << "  -c <config.root>                 configuration file" << endl;
    cout << "  -o <digiout.root>                digitalized event file" << endl;
    cout << "  -t <IHEP|DPNC>                   the type of output file" << endl;
...
void OptionsManager::print_version() {
    cout << endl;
    cout << "    " << SW_NAME << " - POLAR Simulation" << endl;
    cout << "    " << SW_VERSION << " (" << RELEASE_DATE << ", compiled " << __DATE__ << " " << __TIME__ << ")" << endl;
    cout << endl;
    cout << " Copyright (C) 2016-2017 POLAR Simulation Group" << endl;
    cout << endl;
