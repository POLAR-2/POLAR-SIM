...
OutDataFile_IHEP::OutDataFile_IHEP() {
    t_file_out_ = NULL;
...
void OutDataFile_IHEP::clear_data_() {
    // from SCI
...
bool OutDataFile_IHEP::open(const char* filename) {
    if (t_file_out_ != NULL)
        return false;
...
void OutDataFile_IHEP::write_event(const Digi::GeneralEvent_T& general_event) {
    clear_data_();
...
void OutDataFile_IHEP::write_tree() {
    if (t_file_out_ == NULL) {
        return;
    } else {
        t_file_out_->cd();
...
long int OutDataFile_IHEP::get_total_events() {
