...
void POLARRun::RecordEvent(const G4Event* anEvent) {
    ...
    POLARHitsCollection* fPOLARHitsCollection = NULL;
    G4int numberOfHits = 0;
    if (!fPOLARGlobalConfig->primary_only) {
        G4int collectionID = G4SDManager::GetSDMpointer()->GetCollectionID("POLARHitsCollection");
        fPOLARHitsCollection = static_cast<POLARHitsCollection*>(anEvent->GetHCofThisEvent()->GetHC(collectionID));
        if (fPOLARHitsCollection == NULL) {
            G4ExceptionDescription msg;
            msg << "Cannot access hitsCollection: POLARHitsCollection with ID " << collectionID;
            G4Exception("POLAREventAction::EndOfEventAction", "MyCode0001", FatalException, msg);
        }
        numberOfHits = fPOLARHitsCollection->entries();
        ...
        for (int i = 0; i < numberOfHits; i++) {
            POLARHit* aHit = static_cast<POLARHit*>(fPOLARHitsCollection->GetHit(i));
        ...
        if (max_energy_dep < fPOLARGlobalConfig->bar_threshold) return;
    }
    ...
    if (anEvent->GetNumberOfPrimaryVertex() > 1) {
        ...
        G4Exception("POLAREventAction::EndOfEventAction", "MyCode0002", FatalException, msg);
    }
    ...
    if (fPOLARGlobalConfig->primary_only) return;

    if (fPOLARGlobalConfig->event_verbose > 1)
...
void POLARRun::Merge(const G4Run* aRun) {
    G4Run::Merge(aRun);

    const POLARRun* localRun = static_cast<const POLARRun*>(aRun);
