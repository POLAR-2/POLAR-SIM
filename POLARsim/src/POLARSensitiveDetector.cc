...
POLARSensitiveDetector::POLARSensitiveDetector(G4String SDName, G4String HCName): G4VSensitiveDetector(SDName) {
    collectionName.insert(HCName);
    fPOLARHitsCollection_ = NULL;
    fEmSaturation_ = new G4EmSaturation(0);
}

POLARSensitiveDetector::~POLARSensitiveDetector() {
    delete fEmSaturation_;
    fEmSaturation_ = NULL;
}

void POLARSensitiveDetector::Initialize(G4HCofThisEvent* hitCollection) {
    // at the start of an event
    fPOLARHitsCollection_ = new POLARHitsCollection(SensitiveDetectorName, collectionName[0]);
    G4int HCID = G4SDManager::GetSDMpointer()->GetCollectionID(collectionName[0]);
    hitCollection->AddHitsCollection(HCID, fPOLARHitsCollection_);
}

G4bool POLARSensitiveDetector::ProcessHits(G4Step* aStep, G4TouchableHistory* ) {
    // cut hit threshold
    POLARGlobalConfig* fPOLARGlobalConfig = POLARGlobalConfig::Instance();
    if (aStep->GetTotalEnergyDeposit() < fPOLARGlobalConfig->hit_threshold)
        return false;
    // track info
    ...
    // generate a hit
    POLARHit* aHit = new POLARHit();
    if (aHit == NULL) {
        G4ExceptionDescription msg;
        msg << "Cannot access hit " << G4endl;
        G4Exception("POLARSensitiveDetector::ProcessHits()", "MyCode0003", FatalException, msg);
    }
    ...
    fPOLARHitsCollection_->insert(aHit);

    return true;
}

void POLARSensitiveDetector::EndOfEvent(G4HCofThisEvent* ) {
