...
#include "POLARConfigMessenger.hh"

#include "G4SystemOfUnits.hh"
#include "G4PhysicalConstants.hh"

POLARGlobalConfig* POLARGlobalConfig::fgInstance_ = NULL;

POLARGlobalConfig::POLARGlobalConfig() {
    if (fgInstance_ != NULL) {
        G4ExceptionDescription description;
        description << "      "
                    << "POLARGlobalConfig already exists."
                    <<  "Cannot create another instance.";
        G4Exception("POLARGlobalConfig::POLARGlobalConfig()", "Config_F001",
                FatalException, description);
    }
    fPOLARConfigMessenger_ = new POLARConfigMessenger(this);
    fgInstance_ = this;
...
POLARGlobalConfig* POLARGlobalConfig::Instance() {
    if (fgInstance_ == NULL) {
        fgInstance_ = new POLARGlobalConfig();
    }
    return fgInstance_;
}

POLARGlobalConfig::~POLARGlobalConfig() {
    delete fPOLARConfigMessenger_;
}

void POLARGlobalConfig::print_config() {
    G4cout << "=============================================================" << G4endl;
